...
Стандартная библиотека обеспечивает поддержку широко распространенных
математических операций для типов с плавающей запятой, комплексных чисел
и матриц. В нижеследующих разделах приводится краткое введение в эти
математические операции.

Элементарные функции
--------------------

Пакет :ada:`Ada.Numerics.Elementary_Functions` обеспечивает общепринятые
...
Здесь мы используем стандартные константы :ada:`e` и :ada:`Pi` из пакета
:ada:`Ada.Numerics`.

Пакет :ada:`Ada.Numerics.Elementary_Functions` предоставляет операции для типа
:ada:`Float`. Аналогичные пакеты есть для типов :ada:`Long_Float` и
:ada:`Long_Long_Float`.
Например, пакет :ada:`Ada.Numerics.Long_Elementary_Functions` предлагает тот
же набор операций для типа :ada:`Long_Float`.
Кроме того, пакет :ada:`Ada.Numerics.Generic_Elementary_Functions` - это
настраиваемая версия пакета, которую можно использовать для пользовательских
типов с плавающей запятой. Фактически, пакет :ada:`Elementary_Functions`
...
Пакет :ada:`Ada.Numerics.Float_Random` предоставляет простой генератор
случайных чисел с диапазоном от 0,0 до 1,0. Чтобы использовать его, объявите
генератор :ada:`G`, который вы затем передадите в :ada:`Random`. Например:
...
:ada:`Ada.Numerics.Discrete_Random`. Поскольку это
...
Здесь пакет :ada:`R` создается с типом :ada:`Random_Range` с
...
Пакет :ada:`Ada.Numerics.Complex_Types` обеспечивает поддержку комплексных
чисел, а пакет :ada:`Ada.Numerics.Complex_Elementary_Functions`
обеспечивает поддержку общепринятые операций с типами комплексных чисел,
аналогично пакету :ada:`Ada.Numerics.Elementary_Functions`.
Наконец, вы можете использовать пакет :ada:`Ada.Text_IO.Complex_IO` для
выполнения операций ввода-вывода над комплексными числами. В следующем
примере мы объявляем переменные типа :ada:`Complex` и инициализируем
...
Как видно из этого примера, все общепринятые операции, такие, как :ada:`*`
и :ada:`+`, доступны для комплексных типов. У вас также есть типичные операции
комплексных чисел, такие как :ada:`Argument` и :ada:`Exp`. Помимо инициализации
комплексных чисел в декартовой форме с использованием агрегатов, вы можете
получать их из полярной формы, вызвав функцию :ada:`Compose_From_Polar`.

Пакеты :ada:`Ada.Numerics.Complex_Types` и
:ada:`Ada.Numerics.Complex_Elementary_Functions` предоставляют операции для
типа :ada:`Float`. Подобные пакеты доступны для типов :ada:`Long_Float` и
:ada:`Long_Long_Float`. Кроме того, с помощью настраиваемых пакетов
:ada:`Ada.Numerics.Generic_Complex_Types` и
...
Пакет :ada:`Ada.Numerics.Real_Arrays` обеспечивает поддержку векторов и матриц.
...
Если вы не указываете размеры матрицы, они автоматически определяются на основе
агрегата, используемого для инициализации. Хотя вы также можете использовать
явные диапазоны. Например:
...
Пакет :ada:`Ada.Numerics.Real_Arrays` предоставляет операции для типа
:ada:`Float`. Аналогичные пакеты доступны для типов :ada:`Long_Float` и
:ada:`Long_Long_Float`.
Кроме того, настраиваемый пакет :ada:`Ada.Numerics.Generic_Real_Arrays`
