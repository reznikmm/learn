...
Ада позволяет нам взаимодействовать с кодом на многих языках, включая
...
проекта, используемый :program:`gprbuild`. Мы добавляем запись
...
:ada:`C_Enum`, для соответствующего типа в исходном файле C:
...
соответствующие типы данных C (:c:`int`, :c:`long`, :c:`unsigned` и
:c:`double`). А вот объявление в C:
...
Мы можем связать этот код с кодом на Аде, указывая аспект :ada:`Import`.
Например:
...
Ада коде. Например, можно назвать функцию :ada:`Get_Value`:
...
с помощью аспекта :ada:`Export`. Например:
...
же метод, что и для подпрограмм: мы указываем аспекты :ada:`Import` и
...
Мы можем использовать аспект :ada:`External_Name`, если хотим сослаться
...
это с помощью аспекта :ada:`Export`.

Давайте повторно воспользуемся прошлым примером и добавим счетчик,
...
*дампа спецификации Ада*: ``-fdump-ada-spec``. Мы проиллюстрируем
...
Результатом является файл спецификации Ада с именем :file:`test_h.ads`:
...
Теперь мы просто ссылаемся на этот пакет :file:`test_h` в нашем приложении Ада:
...
И получим файл :file:`ext_c_code-test_h.ads`:
...
компилятор может использовать :ada:`System.Address` как тип одного
...
:c:`struct test`, и
все ссылки на структуру :c:`test` заменяются адресами
(:ada:`System.Address`). Тем не менее, эти
...
Мы можем успешно собрать такую программу, используя автоматически
...
#. Определить тип :ada:`Test` на основе :ada:`System.Address` и
   использовать его во всех соответствующих функциях.

#. Удалить префикс ``test_`` во всех операциях с типом :ada:`Test`.
