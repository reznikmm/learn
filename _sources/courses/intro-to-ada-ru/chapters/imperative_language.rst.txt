...
-  Выражение выбора (здесь переменная :ada:`N`) должно быть дискретного типа,
   то есть либо целочисленного типа, либо типа перечисления.
   :ref:`Дискретные типы <Intro_Ada_Ru_WhatIsAType>` будут рассмотрены более
   подробно позже.
...
.. attention::
    ...
    является :ref:`неограниченным типом массива <Intro_Ada_Ru_UnconstrainedArrayTypes>`.
