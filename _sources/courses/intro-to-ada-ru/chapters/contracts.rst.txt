...
вида :ada:`in`, вызывающий :ada:`Op` знает, что аргумент
:ada:`in` не будет изменен :ada:`Op`. Другими словами, вызывающий ожидает,
что :ada:`Op` не изменяет предоставляемый им аргумент, а просто читает
...
*Контрактное программирование* основывается на таких техниках, как
указание пред- и постусловий, предикатов подтипов и инвариантов типов.
...
перед вызовом подпрограммы :ada:`Op`, это предварительные
...
объявлении подпрограммы. Спуцификация :ada:`with Pre => <условие>`
определяет предварительное условие, а спецификация
:ada:`with Post => <условие>` определяет постусловие.
...
.. admonition:: В наборе инструментов GNAT
    ...
    GNAT обрабатывает предварительные и постусловия, генерируя код для
    проверки утверждений (assertion) во время выполнения программы.
    Однако по умолчанию проверка утверждения не включена. Следовательно,
    чтобы проверять предварительные и постусловия во время выполнения,
    вам необходимо включить проверку утверждений с помощью ключа `-gnata`.
...
:ada:`(for all I in A'Range => <условие для A(I)>`, где
:ada:`A` - это массив, а :ada:`I` - индекс. Кванторные выражения,
...
Другой вид кванторных выражений использует :ada:`for some`. Он выглядит
примерно так: :ada:`(for some I in A'Range => <условие для A(I)>`.
...
Мы объявляем 8-битный тип со знаком :ada:`Int_8` и массив элементов этого типа
(:ada:`Int_8_Array`). Мы хотим убедиться, что  после вызова процедуры
:ada:`Square` каждый элемент массива :ada:`Int_8_Array` возведен в квадрат.
Мы делаем это с помощью постусловия, используя выражение :ada:`for all`.
Это постусловие использует атрибут :ada:`'Old` чтобы сослаться на исходное
...
применяя атрибут :ada:`'Result` к именем функции и сравниваем его с входным
значением.
...
функции :ada:`Square` не вызовет переполнения типа :ada:`Int_8` в
нашей функции. Мы делаем это путем преобразования входного значения в
тип :ada:`Integer`, который используется для промежуточных вычислений,
и проверяем, находится ли результат в допустимом для типа :ada:`Int_8`
...
-  :ada:`with Static_Predicate => <свойство>`

-  :ada:`with Dynamic_Predicate => <свойство>`
...
:ada:`Course`, который будет действовать для каждого объекта. Предикат
...
неограниченные строки и даты. Оба типа
доступны в стандартной библиотеке Ада. Пожалуйста, обратитесь к
следующим разделам для получения дополнительной информации:

-  о типе неограниченной строки (:ada:`Unbounded_String`): раздел
   :ref:`Неограниченные строки <Intro_Ada_Ru_UnboundedStrings>`;

-  о дате и времени: Раздел :ref:`Даты и время <Intro_Ada_Ru_DatesTimes>`.
...
Диапазоны в Аде могут быть указаны только как непрерывные списки: они
не позволяют нам выбирать определенные дни. Однако нам может понадобиться
...
три дня в рабочей неделе. Эти дни указаны в подтипе :ada:`Test_Days`. Мы хотим
...
вызове :ada:`Display_Tests` в последнем операторе нашего примера.
...
ожиданий в отношении *личных типов*. Если тип :ada:`T` из пакета :ada:`P`
...
:ada:`with Type_Invariant => <свойство>`. Подобно
предикатам, *свойство* определяет условие, которое позволяет нам
контролировать, соответствует ли объект типа :ada:`T` нашим требованиям.
...
|            | подпрограммы                 |                         |
+============+==============================+=========================+
| Предикаты  | По всем входящим :ada:`in`   | При присваивании и      |
|            | и выходящим :ada:`out`       | явных инициализациях    |
|            | параметрам                   |                         |
+------------+------------------------------+-------------------------+
| Инварианты | По параметрам :ada:`out`,    | При всех инициализациях |
| типов      | возвращенным из подпрограмм, |                         |
|            | объявленных в том же         |                         |
|            | видимом разделе              |                         |
+------------+------------------------------+-------------------------+
...
Основное отличие состоит в том, что в предыдущем примере тип :ada:`Course`
был полностью описан в видимом разделе пакета :ada:`Courses`, но в этом
