...
-  Подход *реального времени*, который лучше подходит для приложений
   реального времени, требующих повышенной точности, например, благодаря
   доступу к абсолютным часам и обработке интервалов времени. Следует
   отметить, что этот подход поддерживает только время, но не даты.

Эти два подхода представлены в следующих разделах.

.. _Intro_Ada_Ru_DatesTimes:
...
получить каждую компоненту с помощью функции :ada:`Split`. Например:
...
Вы делаете это с помощью оператора :ada:`delay until`. Например:
...
дата находится в прошлом, задержка :ada:`delay until` не даст
...
помощью значения типа :ada:`String`. Мы можем сделать это использовав вызов :ada:`Value` из
пакета :ada:`Ada.Calendar.Formatting`. Вот модифицированный код:
...
В приведенных выше примерах мы приостанавливались до определенной даты и
...
используем его в операторе :ada:`delay until`.
...
более тончное значение, вызвав вместо этого :ada:`Nanoseconds`. Кроме того,
нам нужно сначала преобразовать :ada:`D` в тип :ada:`Duration` с помощью
функции :ada:`To_Duration`, прежде чем мы сможем его напечатать.
...
соответствует реботе оператора задержки :ada:`delay`.
